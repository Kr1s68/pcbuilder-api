...
async function getPrices(ItemModel, Price) {
  ...
  const response = await axios.get(
    `https://www.pazaruvaj.com/CategorySearch.php?orderby=1&st=${ItemModel.replace(
      /\s/g,
      "+"
    )}`
  );
  ...
  $("div.optoffer").each((i, element) => {
    const price = parseInt(
      $(element)
        .find("div.row-price")
        .text()
        .substring(0, $(element).find("div.row-price").text().charAt("лв") + 1)
    );
    //console.log(price)
...
app.post("/api/data/createGpu", (req, res) => {
  ...
  const BoostCoreFrequency =
    "Честота на ядрото под натоварване: " + req.body.BoostCoreFrequency + ", ";
...
app.post("/api/data/checkAdmin", (req, res) => {
  ...
  db.query(
    `SELECT * FROM users WHERE email ="${req.body.email}" and admin=1`,
    (err, result) => {
...
app.get("/api/data/loadArticle", (req, res) => {
  db.query(
    `SELECT * FROM article WHERE idarticle = ${req.query.id}`,
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
app.get("/api/data/loadBuild", (req, res) => {
  db.query(
    `SELECT * FROM savedbuilds WHERE user = "${req.query.email}"`,
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
